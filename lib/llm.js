...
window.LLM = (function () {
  ...
  var PRICING = {
    ...
    // GPT-5.6 系列
    'gpt-5.6-sol': { input: 0.010, output: 0.040 },
    'gpt-5.6-terra': { input: 0.005, output: 0.015 },
    'gpt-5.6-luna': { input: 0.0005, output: 0.002 },
    // GPT-5.5 系列
    'gpt-5.5': { input: 0.008, output: 0.024 },
    'gpt-5.5-pro': { input: 0.015, output: 0.060 },
    // GPT-5.4 系列
   ...
   * @param {string} [baseUrl] - 自定义 API 中转地址（不含 /v1/messages）
   * @returns {Promise<string>} LLM 输出的文本
   */
  function callAnthropic(apiKey, model, systemPrompt, userPrompt, jsonMode, baseUrl) {
    var apiUrl = (baseUrl || ANTHROPIC_URL);
    // 如果传入了自定义 baseUrl，需要补全路径
    if (baseUrl) {
      apiUrl = baseUrl.replace(/\/+$/, '') + '/v1/messages';
    }
    var headers = {
      'x-api-key': apiKey,
   ...
   * @param {string} [baseUrl] - 自定义 API 中转地址（不含 /v1/chat/completions）
   * @returns {Promise<string>} LLM 输出的文本
   */
  function callOpenAI(apiKey, model, systemPrompt, userPrompt, jsonMode, baseUrl) {
    var apiUrl = OPENAI_URL;
    if (baseUrl) {
      // 用户填的地址可能带 /v1 也可能不带，统一处理
   ...
   */
  function process(apiKey, model, systemPrompt, userPrompt, baseUrl) {
    // 参数校验
    if (!apiKey) {
    ...
    if (isAnthropicModel(model)) {
      promise = callAnthropic(apiKey, model, systemPrompt, userPrompt, false, baseUrl);
    } else if (isOpenAIModel(model)) {
      promise = callOpenAI(apiKey, model, systemPrompt, userPrompt, false, baseUrl);
    } else {
   ...
   */
  function processChunk(apiKey, model, chunkText, style, baseUrl) {
    if (!chunkText) {
    ...
    var callPromise;
    if (isAnthropicModel(model)) {
      callPromise = callAnthropic(apiKey, model, systemPrompt, userPrompt, true, baseUrl);
    } else if (isOpenAIModel(model)) {
      callPromise = callOpenAI(apiKey, model, systemPrompt, userPrompt, true, baseUrl);
    } else {
      return Promise.reject(new Error('不支持的模型类型：' + model + '（仅支持 claude-* 或 gpt-* 前缀）'));
    }
