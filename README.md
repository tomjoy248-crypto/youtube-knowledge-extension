...
进入 [Releases 发布页面](https://github.com/tomjoy248-crypto/youtube-knowledge-extension/releases)，下载最新的 `KnowledgeView-v1.0.2.zip`，然后解压到本地任意位置。
...
5. 如果在扩展列表中看到「知视 KnowledgeView」卡片（版本号 1.0.2），说明安装成功
...
**Q：粘贴地址后提示字幕为空？**
确认视频提供字幕，然后刷新视频页重试。`v1.0.2` 会同时尝试转写面板、JSON3、XML 和 WebVTT 字幕格式，兼容 YouTube 当前的延迟加载。
