...
进入 [Releases 发布页面](https://github.com/tomjoy248-crypto/youtube-knowledge-extension/releases)，下载最新的 `KnowledgeView-v1.0.10.zip`，然后解压到本地任意位置。
...
5. 如果在扩展列表中看到「知视 KnowledgeView」卡片（版本号 1.0.7），说明安装成功
...
**Q：粘贴地址后提示字幕为空？**
确认视频提供 YouTube CC 字幕，然后刷新视频页重试。`v1.0.10` 会优先通过 YouTube 移动端播放器响应获取可用字幕地址，再按英文优先顺序读取 JSON3、XML 或 WebVTT；只有字幕接口不可用时才打开页面转写面板，并穿透开放 Shadow DOM 查找按钮、标签和字幕分段。这样可避开网页端字幕地址返回空内容的问题，也不会先等待面板超时。
