...
进入 [Releases 发布页面](https://github.com/tomjoy248-crypto/youtube-knowledge-extension/releases)，下载最新的 `KnowledgeView-v1.0.9.zip`，然后解压到本地任意位置。
...
**Q：粘贴地址后提示字幕为空？**
确认视频提供字幕，然后刷新视频页重试。`v1.0.9` 会先展开视频简介，再优先读取简介里的转写入口；如果没有找到，再回退到视频菜单中的转写入口，并完整滚动采集 YouTube 转写面板，同时尝试 JSON3、XML 和 WebVTT 字幕格式。若字幕首尾覆盖不完整，会明确报错而不会继续生成残缺内容。
